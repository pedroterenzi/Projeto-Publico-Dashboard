...
# --- NOVO: FUNÇÃO PARA CONVERTER FRAÇÃO (19/20) EM DECIMAL (1,95) ---
    ...
    try:
        ...
        for choice in data.get('markets', []):
            if choice.get('marketName') == 'Full time' or choice.get('marketId') == 1:
                ...
                o1 = ox = o2 = "---"
                for o in odds:
                    # Pegamos o valor bruto e passamos pela nossa função de conversão
                    valor_bruto = o.get('fractionalValue') if o.get('fractionalValue') else o.get('value')
                    valor_final = converter_odd_decimal(valor_bruto)
                    
                    if o.get('name') == '1': o1 = valor_final
                    if o.get('name') == 'X': ox = valor_final
                    if o.get('name') == '2': o2 = valor_final
                return o1, ox, o2
    except:
        pass
    return "---", "---", "---"
...
prognosticos_dia = [
    {
        ...
        "tendencia": "Cenário para Back Madrid HT ou Over 0.5 Gols HT caso o jogo comece muito aberto.",
        "edge": "Estatística indica valor na vitória do Madrid combinada com +1.5 gols no jogo."
    }
]
...
if st.session_state.page == 'landing':
    ...
    with c_img:
        ...
        except: st.info("🖼️ [Visual Dashboard]")

elif st.session_state.page == 'login':
    c1, c2, c3 = st.columns([1, 2, 1])
    with c2:
        st.markdown("<div style='text-align: center; margin-top: 50px;'><h1 style='color: #10b981;'>💎 LOGIN</h1></div>", unsafe_allow_html=True)
    ...
    # --- BLOCO DASHBOARD (PRECISA DE ARQUIVO) ---
        ...
        if uploaded_file is not None:
            try:
                ...
                if menu == "📈 Performance Geral":
                    ...
                    if len(p_perf) == 2:
                        ...
                        with col1:
                            ...
                            for _, row in res.sort_values('Lucro', ascending=False).iterrows():
                                st.markdown(f"<div class='perf-card'><div><b>{row['Metodo']}</b></div><div style='text-align:right;'><span class='val-pos'>{format_br(row['Lucro'])}</span></div></div>", unsafe_allow_html=True)
                    ...
                    st.markdown("<h2 style='color: white;'>🔥 Domine a Variância</h2>", unsafe_allow_html=True)
...
if menu == "📖 Como Extrair":
    ...
    c1, c2 = st.columns(2)
    with c1: st.markdown('<div class="step-box"><h3>1️⃣ Acesse sua Conta</h3><p>Vá em Atividade da Conta.</p></div>', unsafe_allow_html=True)
