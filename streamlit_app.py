...
# --- FUNÇÃO PARA CONVERTER ODD FRACIONÁRIA EM DECIMAL (BRASIL) ---
def converter_odd_decimal(valor):
    ...
    try:
        ...
        if "/" in valor_str:
            ...
            # Fórmula: (Numerador / Denominador) + 1
            decimal = (float(num) / float(den)) + 1
            return f"{decimal:.2f}".replace(".", ",")
        # Se já for decimal mas vier com ponto
        elif "." in valor_str:
...
def buscar_odds_evento(event_id):
    try:
        ...
        for choice in data.get('markets', []):
            if choice.get('marketName').lower() in ['full time', '1x2'] or choice.get('marketId') == 1:
                odds_lista = choice.get('choices', [])
                o1 = ox = o2 = "---"
                for o in odds_lista:
                    # Forçamos a conversão do valor bruto (fracionário ou decimal)
        ...
        if 'events' in data:
            for event in data.get('events', []):
                ...
                if dt_br.date() == agora_br.date():
                    ...
                    # Busca as Odds reais via API e converte para decimal
...
# =========================================================
    ...
    # --- LÓGICA DE DADOS (DASHBOARD ORIGINAL) ---
    if menu in ["📈 Performance Geral", "📅 Diário de Operações", "📋 Log de Entradas", "📊 Evolução Patrimonial", "⏰ Análise de Janelas", "🔥 Sequências"]:
