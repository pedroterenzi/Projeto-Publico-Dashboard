...
# --- ESTILIZAÇÃO CSS PREMIUM COMPLETA ---
st.markdown("""
    <style>
...
# --- ESTADOS DE SESSÃO (PERSISTÊNCIA) ---
...
# --- BARRA LATERAL ---
with st.sidebar:
    st.markdown("<h2 style='color: #10b981;'>💎 BET PRO</h2>", unsafe_allow_html=True)
    if st.button("Sair"):
        st.session_state.auth = False
        st.rerun()
    st.markdown("---")
    uploaded_file = st.file_uploader("1. Extrato Betfair (.csv)", type=["csv"])
...
if uploaded_file is not None:
    try:
        ...
        # Mapeamento Inteligente (Desktop/Celular/EN/PT)
        ...
        # Função para buscar método classificado
        def get_metodo(row):
        ...
        if menu == "📈 Performance Geral":
            ...
            win_rate = (len(df_clean[df_clean['V_F'] > 0]) / entradas * 100) if entradas > 0 else 0
            ...
            with c1: st.markdown(f'<div class="metric-card" style="background: {bg};"><div class="metric-title">Lucro Líquido</div><div class="metric-value">{format_br(total_l)}</div></div>', unsafe_allow_html=True)
            with c2: st.markdown(f'<div class="metric-card"><div class="metric-title">Taxa de Acerto</div><div class="metric-value">{win_rate:.1f}%</div></div>', unsafe_allow_html=True)
            ...
            with c4: st.markdown(f'<div class="metric-card"><div class="metric-title">Entradas</div><div class="metric-value">{entradas}</div></div>', unsafe_allow_html=True)
            with c5: st.markdown(f'<div class="metric-card"><div class="metric-title">Odd Média</div><div class="metric-value">{odd_m:.2f}</div></div>', unsafe_allow_html=True)
            ...
            with col1:
                ...
                for _, row in res.sort_values('Lucro', ascending=False).iterrows():
                    ...
                    st.markdown(f'<div class="perf-card"><div><b>{row["Metodo"]}</b><br><small>{int(row["Qtd"])} entr. | WR: {wr_m:.1f}%</small></div><div style="text-align:right;"><span class="{cor}">{format_br(row["Lucro"])}</span><br><small>ROI: {(row["Lucro"]/(row["Qtd"]*stake_padrao))*100:.1f}%</small></div></div>', unsafe_allow_html=True)
            ...
            with col2:
                ...
                for _, row in res_odd.iterrows():
                    ...
                    cor = "val-pos" if row['Lucro'] >= 0 else "val-neg"
                    st.markdown(f'<div class="perf-card"><div><b>Odd: {row["Range"]}</b><br><small>{int(row["Qtd"])} entr. | WR: {wr_o:.1f}%</small></div><div style="text-align:right;"><span class="{cor}">{format_br(row["Lucro"])}</span></div></div>', unsafe_allow_html=True)
            ...
            search = st.text_input("Filtrar jogo/mercado")
            ...
            itens = 20
            t_pag = (len(df_v) // itens) + 1
            col_p1, col_p2 = st.columns([1, 4])
            with col_p1:
                p_nav = st.number_input(f"Página (1-{t_pag})", 1, t_pag, key="p_nav")
            ...
            for idx, row in df_v.iloc[start:end].iterrows():
                with st.expander(f"{row['Dt_Obj'].strftime('%d/%m %H:%M')} | {row[c_desc][:55]}... | {format_br(row['V_F'])}"):
            ...
            with c1:
                ...
                for _, row in res_d.iterrows():
                    st.markdown(f'<div class="perf-card"><div><b>{d_s[row["Dia"]]}</b><br><small>{int(row["Qtd"])} entr.</small></div><div style="text-align:right;"><span class="{"val-pos" if row["Lucro"]>=0 else "val-neg"}">{format_br(row["Lucro"])}</span><br><small>ROI: {(row["Lucro"]/(row["Qtd"]*stake_padrao))*100:.1f}%</small></div></div>', unsafe_allow_html=True)
            with c2:
                ...
                for _, row in res_h.iterrows():
                    st.markdown(f'<div class="perf-card"><div><b>{row["Faixa"]}</b><br><small>{int(row["Qtd"])} entr.</small></div><div style="text-align:right;"><span class="{"val-pos" if row["Lucro"]>=0 else "val-neg"}">{format_br(row["Lucro"])}</span><br><small>ROI: {(row["Lucro"]/(row["Qtd"]*stake_padrao))*100:.1f}%</small></div></div>', unsafe_allow_html=True)
